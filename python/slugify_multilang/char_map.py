"""Port of C# ``SlugifyHelper.BuildCharMap()``.

The global character map. Every entry is transcribed verbatim from the C#
dictionary initializer, in the same order, with identical keys and values.
...
def build_char_map() -> dict[str, str]:
    return {
        # Symbols & special
        ...
        # Diacritics are \p{M} and will be stripped by Strict, which is fine for slug purposes.
